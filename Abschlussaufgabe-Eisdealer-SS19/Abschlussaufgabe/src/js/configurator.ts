namespace eisdealer {
    ...
    function addCategory(_categoryName: string, _categoryType: string): HTMLElement {
        ...
        let divCategory: HTMLDivElement = <HTMLDivElement>newElements("div", "category mb-5 border-bottom pb-2", divAllCategories);
        let divCategoryRow: HTMLDivElement = <HTMLDivElement>newElements("div", "category-row row py-2 mb-3", divCategory);
        let divRowCol: HTMLDivElement = <HTMLDivElement>newElements("div", "col-12 col-lg-9 mb-2 mb-lg-0", divCategoryRow);
        let divInputGroup: HTMLDivElement = <HTMLDivElement>newElements("div", "input-group input-group-lg", divRowCol);


        // INPUT GROUP

        let inputCategoryName: HTMLInputElement = <HTMLInputElement>newElements("input", "form-control", divInputGroup);
        ...
        let selectCategoryType: HTMLSelectElement = <HTMLSelectElement>newElements("select", "custom-select", divInputGroup);

        for (let i: number = 0; i < categoryOptions.length; i++) {
            let option: HTMLOptionElement = <HTMLOptionElement>newElements("option", "", selectCategoryType);
        ...
        let divButtonRemoveCategory: HTMLDivElement = <HTMLDivElement>newElements("div", "col-12 col-lg-3 justify-content-end d-flex", divCategoryRow);
        let buttonRemoveCategory: HTMLButtonElement = <HTMLButtonElement>newElements("button", "btn btn-outline-danger w-100", divButtonRemoveCategory);
        ...
        let divItemWrapper: HTMLDivElement = <HTMLDivElement>newElements("div", "items-wrapper", divCategory);

        
        // ADD ITEM BUTTON 

        let divAddItemRow: HTMLDivElement = <HTMLDivElement>newElements("div", "add-item-row row py-2 justify-content-end", divItemWrapper);
        let divAddItemCol: HTMLDivElement = <HTMLDivElement>newElements("div", "col-lg-2 justify-content-end d-flex", divAddItemRow);

        let buttonAddItem: HTMLButtonElement = <HTMLButtonElement>newElements("button", "add-item btn btn-outline-success w-100", divAddItemCol);
	...
	*/
    ...
    function addItem(_targetWrapper: HTMLDivElement, _itemName: string, _itemStock: string, _itemPrice: string): void {
        ...
        let divItemRow: HTMLDivElement = <HTMLDivElement>newElements("div", "row py-2 justify-content-start", null);

        _targetWrapper.insertBefore(divItemRow, _targetWrapper.children[(_targetWrapper.children.length - 1)]);

        let divItemRowCol1: HTMLDivElement = <HTMLDivElement>newElements("div", "col-lg-1 mb-2 mb-lg-0", divItemRow);
        let divItemRowCol9: HTMLDivElement = <HTMLDivElement>newElements("div", "col-lg-9 mb-2 mb-lg-0", divItemRow);
        let divItemInputGroup: HTMLDivElement = <HTMLDivElement>newElements("div", "input-group", divItemRowCol9);

        // INPUT GROUP

        let inputItemName: HTMLInputElement = <HTMLInputElement>newElements("input", "form-control", divItemInputGroup);
        inputItemName.setAttribute("type", "text");
        inputItemName.setAttribute("placeholder", "Item-Name");
        if (_itemName != "")
            inputItemName.value = _itemName;

        let inputItemStock: HTMLInputElement = <HTMLInputElement>newElements("input", "form-control", divItemInputGroup);
        inputItemStock.setAttribute("type", "text");
        inputItemStock.setAttribute("placeholder", "Stock");
        if (_itemName != "")
            inputItemStock.value = _itemStock;

        let inputItemPrice: HTMLInputElement = <HTMLInputElement>newElements("input", "form-control", divItemInputGroup);
        inputItemPrice.setAttribute("type", "text");
        inputItemPrice.setAttribute("placeholder", "Price");
        if (_itemName != "")
            inputItemPrice.value = _itemPrice;

        let divSpanAppend: HTMLDivElement = <HTMLDivElement>newElements("div", "input-group-append", divItemInputGroup);
        ...
        let divButtonRemoveItem: HTMLDivElement = <HTMLDivElement>newElements("div", "col-lg-2 justify-content-end d-flex", divItemRow);
        let buttonRemoveItem: HTMLButtonElement = <HTMLButtonElement>newElements("button", "btn btn-outline-danger w-100", divButtonRemoveItem);
